...
foreach ($allInvoices as $inv) {
    ...
    $invoiceData = [
        ...
        'expiry_date'        => (function() use ($transaction) {
            foreach ($transaction['fields'] ?? [] as $f) {
                if (($f['name'] ?? '') === 'Expiry Date') return $f['value'] ?? null;
            }
            return null;
        })(),
    ...
    // ── ALL contact fields ──────────────────────────────────────────────────
    $customFields = array_column($contact['fields'] ?? [], 'value', 'name');
    $contactData = [
        'id'                        => $contact['id']                         ?? null,
        'client_id'                 => $customFields['Client ID']             ?? null,
        ...
        'types'                     => $contact['types']                      ?? [],
        'receivable_account_id'     => $contact['receivable_account_id']      ?? null,
        'debtor_credit_limit'       => $contact['debtor_credit_limit']        ?? null,
        'payable_account_id'        => $contact['payable_account_id']         ?? null,
        'group_ids'                 => $contact['group_ids']                  ?? [],
        'group_names'               => $contact['group_names']                ?? null,
        'price_level_id'            => $contact['price_level_id']             ?? null,
        ...
        'default_currency_code'     => $contact['default_currency_code']      ?? null,
        'default_term_id'           => $contact['default_term_id']            ?? null,
        'default_income_account_id' => $contact['default_income_account_id']  ?? null,
        'default_expense_account_id'=> $contact['default_expense_account_id'] ?? null,
        'default_location_id'       => $contact['default_location_id']        ?? null,
        ...
        'receive_monthly_statement' => $contact['receive_monthly_statement']  ?? null,
        'receive_invoice_reminder'  => $contact['receive_invoice_reminder']   ?? null,
