...
function parseCallEvents(array $events): array {
    ...
    // Check if events[0] exists and extract dialed number
    if (isset($events['events'][0]['response']['response_body'])) {
        $responseBody = $events['events'][0]['response']['response_body'];
    ...
    // Check if events[1] exists and extract dial_call_status
    if (isset($events['events'][1]['request']['parameters']['dial_call_status'])) {
        $dialCallStatus = $events['events'][1]['request']['parameters']['dial_call_status'];
        $dialCallSid = $events['events'][1]['request']['parameters']['dial_call_sid'] ?? null;
        $dialBridged = $events['events'][1]['request']['parameters']['dial_bridged'] ?? null;

        // Update answerPoints with status
        if (!empty($answerPoints)) {
            $answerPoints[0]['call_sid'] = $dialCallSid;
            $answerPoints[0]['status'] = $dialCallStatus;
            $answerPoints[0]['bridged'] = $dialBridged;
            $answeredBy = $answerPoints[0]['number'];
            $answeredByStatus = $dialCallStatus;
        }
    }

    // If no status found, default to no-answer
    if (empty($answerPoints)) {
        $answerPoints[] = [
            'number' => null,
            'call_sid' => null,
        ...
        if (empty($accountSid) || empty($callSid)) {
            ...
            if ($callDetails) {
                ...
                if ($answerPointData['answeredBy']) {
                    ...
                    logMessage("No answeredBy phone number to match with LeadConnector users.");
                }
                ...
                if ($recordings && isset($recordings['recordings']) && count($recordings['recordings']) > 0) {
                    ...
                    $recordingMediaUrls = implode(',', array_filter($urls));
